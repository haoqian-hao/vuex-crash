<template>
    <v-chart class="chart-k" :option="option" autoresize/>
</template>
...
export default {
    ...
    created() {
        axios
            .get("https://raw.githubusercontent.com/apache/echarts-website/asf-site/examples/data/asset/data/stock-DJI.json")
            .then(
                response => {
                    const data = splitData(response.data)
                    this.option =  {
                        animation: false,
                        legend: {
                            bottom: 10,
